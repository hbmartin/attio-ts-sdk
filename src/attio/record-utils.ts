...
export const normalizeRecord = <T extends AttioRecordLike>(
  ...
  const hasValidId =
    raw.id && (raw.id as Record<string, unknown>).record_id;
  ...
  if (!result.id || !(result.id as Record<string, unknown>).record_id) {
    ...
    if (extractedId) {
      result.id = { ...(result.id as Record<string, unknown>), record_id: extractedId };
    }
  }

  if (!result.values) {
    const nested = (result.data as Record<string, unknown>)?.values;
    result.values = (nested as Record<string, unknown>) ?? {};
  }
...
export const normalizeRecords = <T extends AttioRecordLike>(
  ...
  return items
    ...
    .map((item) => normalizeRecord<T>(item as Record<string, unknown>, options));
};
